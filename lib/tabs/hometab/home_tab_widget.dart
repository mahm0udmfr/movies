...
class HomeTabWidgetState extends State<HomeTabWidget> {
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        CachedNetworkImage(
          imageUrl: widget.availableNowList[selectedIndex].mediumCoverImage,
          fit: BoxFit.fill,
          height: double.infinity,
          width: double.infinity,
          progressIndicatorBuilder: (context, url, downloadProgress) =>
              CircularProgressIndicator(value: downloadProgress.progress),
          errorWidget: (context, url, error) => Icon(Icons.error, color: AppColor.white), // Show error icon
        ),
        Container(
            ...
            CarouselSlider.builder(
              ...
              itemBuilder: (BuildContext context, int itemIndex, int pageViewIndex) {
                return Container(
                  ...
                  child: MovieWidget(
                    imageUrl: widget.availableNowList[itemIndex].mediumCoverImage,
                    ratingText: widget.availableNowList[itemIndex].rating.toString(),
                  ),
                );
              },
            ),
